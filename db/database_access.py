...
def db_add_item_using_category_name(session, category_name,
                                    item_name, item_desc, user_id):
        category = session.query(Category).filter_by(name=category_name).one()
        session.add(Item(
            name=item_name, description=item_desc, category_id=category.id))
        session.commit()
...
def db_latest_items(db_session, number_of_items=latest_items):
    latest_items = db_session.query(Item).order_by(Item.created.desc())
    latest_items = latest_items.limit(number_of_items)
    return latest_items
